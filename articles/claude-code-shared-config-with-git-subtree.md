...
個人開発でClaude Codeを使い込んでいくうちに、`.claude/`の中身がそれなりに育ってきた。コマンド、スキル、エージェントと、自分の開発スタイルに合わせた設定が一通り揃った状態。

で、別のリポジトリでも同じ設定を使いたくなった。最初はコピペで持っていったが、元のリポジトリでコマンドを改善するたびに他のリポジトリにも同じ修正を入れる必要があって、すぐに破綻した。

ただ、全部を共通化すればいいという話でもない。`settings.json`やプロジェクト固有のコマンドなど、リポジトリごとに違って当然のものもある。
...
いくつかの方法を検討して、Git Subtree + symlinkという構成に落ち着いた。
