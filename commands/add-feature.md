---
description: 新機能を既存パターンに従って、完全に無停止で実装する (spec-planとimplementのラッパー)
argument-hint: 機能名 または GitHub issue URL
---

# 新機能の追加 (完全自動実行モード)

**このコマンドは、`/spec-plan` と `/implement` を順番に実行するラッパーコマンドです。**
...
## ステップ1: 計画フェーズ (/spec-plan の実行)

1. **引数を解析:**
   - GitHub issue URL または機能名を識別

2. **`/spec-plan` コマンドを実行:**
   - 引数をそのまま `/spec-plan` に渡す
   - `/spec-plan` は以下を実行:
     - issue URL の場合は内容を取得
     - プロジェクト理解と既存パターン調査
...
3. **ステアリングディレクトリパスを取得:**
   - `/spec-plan` が作成したステアリングディレクトリのパスを特定
...
- ステップ1: `/spec-plan` が正常に完了し、ステアリングファイルが作成されている
...
- このコマンドは `/spec-plan` と `/implement` のラッパーです
- 各コマンドの詳細な動作は、それぞれのコマンドファイルを参照してください
- 要件が不明確な場合でも、`/spec-plan` が対話的に確認するため、このコマンドをそのまま実行できます
- 後方互換性のため、既存の動作を維持しています

## 使い分け

- **`/add-feature`**: 計画から実装まで一気に完了させたい場合 (このコマンド)
- **`/spec-plan`**: まず計画だけを立てたい場合、要件を確認したい場合
